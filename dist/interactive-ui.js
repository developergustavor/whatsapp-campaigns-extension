...
let activeMode="text";
let drafts={};
export function resetInteractive(){drafts={};activeMode="text";rows=[];}
...
export function initInteractive(mode,value={}){
 ...
 $('#modeHelp').textContent=({text:'Mensagem com variáveis de nome e número.',invite:'Informe o link do grupo para gerar um convite nativo.',cta:'Defina o texto de cada botão e a ação: abrir URL ou ligar.',quick_reply:'Defina o texto e o ID de cada resposta rápida.',list:'Defina o botão de abertura, as seções e as opções com seus IDs.'})[mode];
...
export function interactiveValue(mode){
 ...
 }else value.buttons=rows.map(r=>mode==='quick_reply'?{id:r.id,text:r.text}:r.kind==='phone'?{phoneNumber:r.value,text:r.text}:{url:r.value,text:r.text});
 return value;
}
...
export function updateContentPreview(){
 ...
 let html=`<div class="message-preview"><strong>${esc(x.title||'')}</strong><p>${esc(body)}</p><small>${esc(x.footer||'')}</small></div>`;
