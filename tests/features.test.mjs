...
test('CTA valida URL e telefone, quick reply exige IDs únicos',()=>{
 validateConfig({...config,mode:'cta',interactive:{buttons:[{text:'Visitar',url:'https://example.com'}]}});
 validateConfig({...config,mode:'cta',interactive:{buttons:[{text:'Ligar',phoneNumber:'+5511999999999'}]}});
 assert.throws(()=>validateConfig({...config,mode:'cta',interactive:{buttons:[{text:'Teste',url:'javascript:alert(1)'}]}}));
...
test('CTA, quick reply e lista são encaminhados ao método correto',async()=>{
 ...
 assert.equal(calls[0][2],'Teste');assert.equal(calls[0][3].useInteractiveMessage,true);assert.equal(calls[1][0],'text');assert.equal(calls[2][0],'list');assert.equal(calls[2][2].description,'Teste');
});
